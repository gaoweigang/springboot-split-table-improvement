...
public class RSAUtil {
    ...
    public static String RSAEncode(String text,String publicKeyStr) {
        X509EncodedKeySpec x509EncodedKeySpec = new X509EncodedKeySpec(java.util.Base64.getDecoder().decode(publicKeyStr));
        try {
            KeyFactory factory = KeyFactory.getInstance(KEY_ALGORITHM);
            PublicKey key = factory.generatePublic(x509EncodedKeySpec);
            Cipher cipher = Cipher.getInstance(KEY_ALGORITHM);
            cipher.init(Cipher.ENCRYPT_MODE, key);
            return java.util.Base64.getEncoder().encodeToString((cipher.doFinal(text.getBytes("UTF-8"))));
        } catch (Exception e) {
        }
        return null;
    }
    ...
    public static String RSADecode(String text,String privateKeyStr) {
        PKCS8EncodedKeySpec pkcs8EncodedKeySpec = new PKCS8EncodedKeySpec(
                java.util.Base64.getDecoder().decode(privateKeyStr));
        try {
            KeyFactory factory = KeyFactory.getInstance(KEY_ALGORITHM);
            PrivateKey key = factory
                    .generatePrivate(pkcs8EncodedKeySpec);
            Cipher cipher = Cipher.getInstance(KEY_ALGORITHM);
            cipher.init(Cipher.DECRYPT_MODE, key);
            return new String(cipher.doFinal(java.util.Base64.getDecoder().decode(text)));
        } catch (Exception e) {
        }
        return null;
